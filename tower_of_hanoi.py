...
def criar_discos(n):
    ...
    for i in range(n):
        ...
        discos.append(disco)
        discos_info.append({'indice': i, 'haste': 'A', 'posicao': i})

def atualizar_ordem(haste):
    discos_na_haste = []
    for i, info in enumerate(discos_info):
        if info['haste'] == haste:
            discos_na_haste.append((i, info['posicao']))
    
    discos_na_haste.sort(key=lambda x: x[1])
    
    for nova_pos, (disco_idx, _) in enumerate(discos_na_haste):
        x_pos = posicoes_hastes[haste]
        novo_y = -165 + 20 * nova_pos
        
        discos_info[disco_idx]['posicao'] = nova_pos
        discos[disco_idx].goto(x_pos, novo_y)
...
def mover_disco(disco_idx, origem, destino):
    global passo_atual
    passo_atual += 1
    contador()
...
def resolver(n, origem, destino, auxiliar):
    if n == 1:
        mover_disco(n-1, origem, destino)
        return
